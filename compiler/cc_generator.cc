...
private:
    std::string res_;
    CcGenerator *generator_;

public:
    ConstToStringVisitor(CcGenerator *generator)
    ...
    {
        res_ = CcGenerator::number(instr->value());
    }
    ...
    {
        res_ = CcGenerator::string(instr->value());
    }
...
std::string CcGenerator::string(const String &str)
{
...
std::string CcGenerator::boolean(bool val)
{
    return val ? "true" : "false";
}

std::string CcGenerator::number(double val)
{
...
std::string CcGenerator::value(ir::Value *val)
{
    if (val->is_constant())
    {
...
std::string CcGenerator::type(const ir::Type *type)
{
...
std::string CcGenerator::allocate(const ir::Type *type, const std::string &name)
{
    std::string str = CcGenerator::type(type);
...
std::string CcGenerator::uint32(uint32_t val)
{
    std::stringstream out;
    out.fill('0');
...
std::string CcGenerator::uint64(uint64_t val)
{
    std::stringstream out;
    out.fill('0');
...
void CcGenerator::visit_block(ir::Block *block)
{
    cur_block_ = block;
...
void CcGenerator::visit_instr_arr(ir::ArrayInstruction *instr)
{
...
void CcGenerator::visit_instr_bin(ir::BinaryInstruction *instr)
{
    switch (instr->operation())
    {
...
void CcGenerator::visit_instr_call(ir::CallInstruction *instr)
{
    std::string kind;
    switch (instr->operation())
    {
...
void CcGenerator::visit_instr_val(ir::ValueInstruction *instr)
{
    switch (instr->operation())
    {
...
void CcGenerator::visit_instr_br(ir::BranchInstruction *instr)
{
...
void CcGenerator::visit_instr_jmp(ir::JumpInstruction *instr)
{
    if (instr != cur_block_->last_instr() ||
...
void CcGenerator::visit_instr_ret(ir::ReturnInstruction *instr)
{
...
void CcGenerator::visit_instr_link(ir::Link *instr)
{
    switch (instr->kind())
    {
...
void CcGenerator::visit_instr_es_bin(ir::EsBinaryInstruction *instr)
{
    switch (instr->operation())
    {
        // Arithmetic.
...
void CcGenerator::visit_instr_es_unary(ir::EsUnaryInstruction *instr)
{
    switch (instr->operation())
    {
...
void CcGenerator::generate(ir::Module *module, const std::string &file_path)
{
